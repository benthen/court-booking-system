...
public class ReservationRequest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (isReserved == 0) {
				AddReservationAndHistory add = new AddReservationAndHistory(id, number, college, court, duration);
				add.addReservationAndHistory(MySQL.getInstance().getSQLConnection());
